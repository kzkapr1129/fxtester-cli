        ...
        if kind == "peak":
            ...
            if lastPeak is not None:
                ...
                if lastPeakBodyMax < bodyMax and lastBottom is not None:
                    df.loc[lastBottom, "breakout"] = True
                    df.loc[lastBottom, "breakout-top"] = min(df.loc[lastBottom, "open"], df.loc[lastBottom, "close"])
                    # 前回高値と今の高値の比率を求める
                    ...
                    df.loc[lastBottom, "breakout-top-rate"] = update_rate
                    df.loc[lastBottom, "breakout-top-dist"] = dist
            ...
            if lastBottom is not None:
                ...
                if bodyMin < lastBottomBodyMin and lastPeak is not None:
                    df.loc[lastPeak, "breakout"] = True
                    df.loc[lastPeak, "breakout-bottom"] = max(df.loc[lastPeak, "open"], df.loc[lastPeak, "close"])
                    # 前回高値と今の高値の比率を求める
                    ...
                    df.loc[lastPeak, "breakout-bottom-rate"] = update_rate
                    df.loc[lastPeak, "breakout-bottom-dist"] = dist
