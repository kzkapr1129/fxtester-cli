...
def show(df: DataFrame, sma: list[int]=[], title: str = ""):
    ...
    # ジグザグのマーカー追加
    apds.append(mpf.make_addplot(df['zigzag-peak-price'], type='scatter', markersize=10, marker='v', color='red'))
    apds.append(mpf.make_addplot(df['zigzag-bottom-price'], type='scatter', markersize=10, marker='^', color='blue'))
